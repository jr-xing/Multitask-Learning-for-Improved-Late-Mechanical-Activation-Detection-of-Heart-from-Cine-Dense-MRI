...
import torch
import tqdm
from modules.visualization.pytorch_grad_cam.base_cam import BaseCAM
        ...
        with torch.no_grad():
            ...
            for batch_index, tensor in enumerate(input_tensors):
                ...
                for i in tqdm.tqdm(range(0, tensor.size(0), BATCH_SIZE)):
                    ...
                    if type(raw_outputs) is tuple:
                        outputs = raw_outputs[0].cpu().numpy()[:, category]
                    else:
                        outputs = raw_outputs.cpu().numpy()[:, category]
